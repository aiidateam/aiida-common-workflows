# -*- coding: utf-8 -*-
"""Implementation of `aiida_common_workflows.common.relax.generator.CommonRelaxInputGenerator` for CP2K-SIRIUS."""
import collections
import pathlib
import typing as t
...
__all__ = ('Cp2kSiriusCommonRelaxInputGenerator',)
...
def get_kinds_section(structure: StructureData, basis_pseudo: str, magnetization_tags):
    """ Write the &KIND section for SIRIUS """
    kinds = []
    with open(pathlib.Path(__file__).parent / basis_pseudo, 'rb') as handler:
        atom_data = yaml.safe_load(handler)
...
def get_file_section(structure: StructureData, basis_pseudo: str):
    """Provide necessary parameter files for CP2K-SIRIUS
    """
    files_dict = {}
    ...
    for symbol, tag in symbol_tag:
        with open(pathlib.Path(__file__).parent / 'pseudopotentials' / atom_data['UPF_pseudopotential'][symbol], 'rb') as handler:
            files_dict[symbol] = orm.SinglefileData(file=handler)
    return files_dict

class Cp2kSiriusCommonRelaxInputGenerator(CommonRelaxInputGenerator):
    """Input generator for the `Cp2kSiriusRelaxWorkChain`."""
    ...
    def _initialize_protocols(self):
        ...
        with open(pathlib.Path(__file__).parent / 'protocol.yml', 'rb') as handler:
    ...
    def _construct_builder(self, **kwargs) -> engine.ProcessBuilder:
        ...
        # Cell in A for SIRIUS
        cell = parameters['FORCE_EVAL']['SUBSYS']['CELL']
        for i, keys in enumerate(cell.keys()):
            cell[keys] = '{} {:<15} {:<15} {:<15}'.format(cell[keys], structure.cell[i][0],structure.cell[i][1],structure.cell[i][2])
        ...
        # Files.
        builder.cp2k.file = get_file_section(structure, basis_pseudo)
