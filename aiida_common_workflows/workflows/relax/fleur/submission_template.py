...
assert set(InpGen.get_protocol('standard')) == {
    ...
    '0.2 ang^-1 and if not additional specification'
    ' are given it uses the default FLAPW determined by FLEUR. This is safe for a default calculation.'
}
assert set(InpGen.get_protocol('testing')) == {
    'description':
    'This is a fast protocol to quickly get some results, typically unconverged. '
    'This uses a k-mesh with inverse linear density of 0.4 ang^-1 and if not additional specification'
    ' are given it uses the default FLAPW determined by FLEUR. This is useful for debugging and tutorials.'
}
